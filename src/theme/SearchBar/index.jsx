...
const Search = props => {
  ...
  const toggleSearchIconClick = useCallback(
    e => {
      // Ensure the index loads even on touch devices / icon-first clicks,
      // where the input's onMouseOver/onFocus lazy-load triggers may never
      // fire on a still-disabled (not-yet-indexReady) input.
      loadAlgolia();
  ...
  // Upstream docusaurus-lunr-search calls loadAlgolia() unconditionally here on
  // every render, which fetches the full search index (~800KB combined) on
  // every single page load regardless of whether the visitor ever opens
  // search. Swizzled locally (see src/theme/SearchBar/) to defer that fetch
  // until the visitor actually focuses/clicks/hovers the search input — those
  // handlers already call loadAlgolia() further down in this file.
  let placeholder
  if (isBrowser) {
